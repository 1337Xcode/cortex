-- Add 'Method' to the allowed node kinds.
-- Task 8.1 introduced NodeKind::Method for functions defined inside
-- class/struct/impl/trait bodies. The CHECK constraint must be updated
-- to accept this new variant.
-- SQLite does not support ALTER TABLE to modify CHECK constraints,
-- so we recreate the table with the expanded constraint.

CREATE TABLE nodes_new (
    fqn TEXT PRIMARY KEY,
    kind TEXT NOT NULL CHECK(kind IN ('Function','Class','Module','Route','Interface','Type','Enum','Constant','TypeAlias','Trait','Namespace','Document','Method')),
    file TEXT NOT NULL,
